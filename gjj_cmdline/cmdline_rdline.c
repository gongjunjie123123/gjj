...
#define MAX_CLI_SUPPT   128
char g_repeat_cmd[MAX_CLI_SUPPT][16];
int g_cmd_idx;

int is_repeat_cli(char *buf)
{
    int i;
    int ret;
    int cli_num = g_cmd_idx % MAX_CLI_SUPPT;
    
    for(i = 0; i < cli_num; i++) {
        ret = strncmp(g_repeat_cmd[i], buf, strlen(buf));
        if (ret == 0) return 1;
    }
    memcpy(g_repeat_cmd[cli_num], buf, strlen(buf));
    g_cmd_idx++;
	...
	if (cmd >= 0) {
		...
		case CMDLINE_KEY_HELP:
			...
			if (rdl->complete) {
                ...
                memset(g_repeat_cmd, 0, sizeof(g_repeat_cmd));
                g_cmd_idx = 0;

				/* choice */
				rdline_puts(rdl, "\r\n");
				while (ret) {
                    if(0 == is_repeat_cli(tmp_buf)) {
    					rdl->write_char(rdl, ' ');
    					for (i=0 ; tmp_buf[i] ; i++)
    						rdl->write_char(rdl, tmp_buf[i]);
    					rdline_puts(rdl, "\r\n");
